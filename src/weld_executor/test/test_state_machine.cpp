...
TEST(StateMachine, AllowsNominalExecutionPath) {
  ...
  EXPECT_TRUE(
      weld_executor::CanTransition(ExecutionState::kValidating, ExecutionState::kPlanning));
  ...
  EXPECT_TRUE(
      weld_executor::CanTransition(ExecutionState::kExecuting, ExecutionState::kCompleted));
}
